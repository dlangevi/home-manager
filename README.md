...
`dlsys upgrade all` runs `sudo nixos-rebuild switch --flake .#$host`
followed by `home-manager switch --flake .#$host`. Use `dlsys upgrade
nixos` for the system layer alone, or `dlsys upgrade` (default `hm`)
