...
let
  ...
  # git repo, edited in place, not generated by Nix. That tree also holds
  # jpc-landing/ and radio/ (former standalone sites, consolidated here)
  # and media/ (the album-requests app's static files, served by the
  # backend itself -- see album-requests.nix / ingestPort below). Root
...
in
{
  ...
  services.nginx = {
    ...
    virtualHosts.${jpcDomain} = {
      ...
      # request.jpc.dlangevi.com folded into a path here -- see the comment
      # on ingestPort above. Bare /media (no trailing slash) redirects first:
      # without that, a request for the bare path never matches the
      # trailing-slash location below, so relative links/assets on the
      # served page would resolve one level too high.
      #
      # jpc-landing/ and radio/ (also part of the frontend checkout now, see
      # landingDir above) aren't routed here yet -- they were dropped from
      # nginx during the consolidation and need their own locations added
      # back when that's decided.
      locations."/media" = {
        return = "301 /media/";
      };
      locations."/media/" = {
        # Trailing slash on both sides of proxyPass strips the /media/
